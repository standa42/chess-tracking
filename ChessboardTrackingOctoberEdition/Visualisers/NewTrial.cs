...
namespace ChessboardTrackingOctoberEdition.Visualisers
{
    class NewTrial : IVisualiser
    {
        ...
        public NewTrial(Form form, IKinect kinect, StateAutomataWrapper stateAutomata)
        {
            this.kinect = kinect;
        ...
        private bool[] resultBools = null;
        private LineSpecial finalEdge = null;
            ...
            DepthSpacePoint[] pointsFromColorToDepth, ColorSpacePoint[] pointsFromDepthToColor)
        {
            ...
            try
            {
                ...
                angles = data.RotationTo2DModified();
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        if (!double.IsInfinity(depthReference.Y) && !double.IsNaN(depthReference.Y))
                        {
                            ...
                            if ((dis1 && dis2))
                            {
                                ...
                                if (csp.Z > -0.014f)// height of threshold
                                {
